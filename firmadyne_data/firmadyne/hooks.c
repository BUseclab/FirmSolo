...
\
	...
	HOOK("send_signal", send_signal_hook, send_signal_probe) \
\
	/* Hook memory mapping */ \
...
static void send_signal_hook(int sig, struct siginfo *info, struct task_struct *t, int group) {
	if (syscall & LEVEL_EXEC) {
		if (sig == 11 || sig == 10){
			printk(KERN_INFO MODULE_NAME": sig_segv[PID: %d (%s)]: PID:%d, signal:%u\n", task_pid_nr(current), current->comm, t->pid, sig);
		}
		else{
			printk(KERN_INFO MODULE_NAME": send_signal[PID: %d (%s)]: PID:%d, signal:%u\n", task_pid_nr(current), current->comm, t->pid, sig);
